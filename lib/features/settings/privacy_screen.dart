  ...
  Future<void> _clearScanCache() async {
    ...
    try {
      ...
      for (final it in items) {
        ...
        if (img != null && img.isNotEmpty) referenced.add(img);
        final thumb = it.thumbPath;
        if (thumb != null && thumb.isNotEmpty) referenced.add(thumb);
      }
      for (final p in photos) {
        if (p.localPath.isNotEmpty) referenced.add(p.localPath);
        final thumb = p.thumbPath;
        if (thumb != null && thumb.isNotEmpty) referenced.add(thumb);
      }

      final deleted = await imageStorage.cleanupOrphanedScanFiles(
