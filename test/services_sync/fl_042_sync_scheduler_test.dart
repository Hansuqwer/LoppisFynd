...
void main() {
  ...
  test('SyncScheduler is a no-op when comps are disabled', () async {
    final db = AppDatabase.inMemory();
    addTearDown(db.close);

    await db.appSettingsDao.setInt(kPrivacyFetchSoldPriceCompsEnabledKeyV1, 0);
    ...
    final before = await db.scanItemsDao.getById('scan-1');
    expect(before, isNotNull);

    final scheduler = SyncScheduler(
      db: db,
      market: _ThrowingMarket(),
      clock: _FakeClock(DateTime(2026, 2, 14, 10)),
      maxCallsPerDay: 10,
    );
    ...
    final after = await db.scanItemsDao.getById('scan-1');
    expect(after, isNotNull);
    expect(after!.status, ScanItemStatus.pendingSync);
